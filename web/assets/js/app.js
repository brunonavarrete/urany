	...
	$('form.newsletter').submit(function(e){
		...
		$.ajax({
	        ...
	        success     : function(data) {
	            if (data.result == "success" || data.msg.indexOf('ya está suscrito a esta lista') >= 0 ) {
	                if( $form.is('.newsletter-subscribe') ){
	                	$form.fadeOut(50);
	                	setTimeout(function(){
	                		$('#download').addClass('success');
	                	},65);
	                	ga('send', {'hitType': 'pageview', 'page': '/descarga-catalogo', 'title': 'Descarga catalogo'});
	                } else {
...
-------------------------- */
	$('.social-goal').click(function(){
		ga('send', {'hitType': 'pageview', 'page': '/social-share', 'title': 'Social share'});
	});
